...
MINIX_ORIG_SRC_COPY=include,lib,man,servers,kernel,commands
...
MINIX_ADD_DISK_EXT2=additional_disk_ext2.img
MINIX_ADD_DISK_SIZE=50M

QEMU=qemu
QEMU_IMG=qemu-img
QEMU_OPTS=-localtime -k fr-be -net user -net nic -m 256 -hda $(MINIX_LOCAL_DISK) -hdb $(MINIX_ADD_DISK) -hdc $(MINIX_ADD_DISK_EXT2)
...
all: 
	@echo "Utilisez 'make <target>' oÃƒÂ¹ <target> est :"
...
init: $(MINIX_LOCAL_DISK) $(MINIX_ADD_DISK) $(MINIX_ADD_DISK_EXT2) $(MINIX_LOCAL_SRC) $(MINIX_LOCAL_ORIG_SRC) $(MINIX_LOCAL_TEST)
	
run: init $(MINIX_LOCAL_DISK) $(MINIX_ADD_DISK)  $(MINIX_ADD_DISK_EXT2)
	@$(QEMU) -curses $(QEMU_OPTS) #2> /dev/null 

run_x11: $(MINIX_LOCAL_DISK) $(MINIX_ADD_DISK)  $(MINIX_ADD_DISK_EXT2)
	@$(QEMU) $(QEMU_OPTS)
...
mrproper: clean
	@rm -f $(MINIX_LOCAL_DISK) $(MINIX_ADD_DISK) $(MINIX_ADD_DISK_EXT2)
...
$(MINIX_ADD_DISK_EXT2):
	@echo -n "Creation et formattage du disque virtuel: $@... "
	@$(QEMU_IMG) create $@ $(MINIX_ADD_DISK_SIZE) > /dev/null 2>/dev/null
	@mkfs.ext2 -F -L "disk_ext2" $@ > /dev/null
	@echo "done."
...
$(PATCH_FILE): $(MINIX_LOCAL_SRC) $(MINIX_LOCAL_ORIG_SRC)
	@echo -n "GÃƒÂ©nÃƒÂ©ration du patch: $@... "
